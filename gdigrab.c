...
static int64_t g_lockscreen_start_time=0;

typedef BOOL(PASCAL * funWTSQuerySessionInformation)(HANDLE hServer, DWORD SessionId, WTS_INFO_CLASS WTSInfoClass, LPTSTR* ppBuffer, DWORD* pBytesReturned);
typedef void (PASCAL * funWTSFreeMemory)(PVOID pMemory);


static funWTSQuerySessionInformation g_pWTSQuerySessionInformation = NULL;
static funWTSFreeMemory g_pWTSFreeMemory = NULL;

static HMODULE g_hLib=NULL;


static void 
gdigrab_hlib_uninit(void)
{
    av_log(NULL, AV_LOG_WARNING, "gdigrab_hlib_uninit \n ");     
    
    if (g_hLib != NULL)
    {
        av_log(NULL, AV_LOG_WARNING, "free g_hLib \n ");     
        
        FreeLibrary(g_hLib);
        g_hLib=NULL;
        g_pWTSQuerySessionInformation=NULL;
        g_pWTSFreeMemory=NULL;
    }
    
}

static void 
gdigrab_hlib_init(void )
{
    av_log(NULL, AV_LOG_WARNING, "gdigrab_hlib_init \n ");     
    if (NULL!=g_hLib)
        return ;
    g_hLib = LoadLibrary("wtsapi32.dll");
    if (g_hLib==NULL)
    {
        av_log(NULL, AV_LOG_WARNING, "hLib null \n ");     
        return ;
    }        
    g_pWTSQuerySessionInformation = (funWTSQuerySessionInformation)GetProcAddress(g_hLib, "WTSQuerySessionInformationW");
    if (g_pWTSQuerySessionInformation==NULL)
        return ;
    
    g_pWTSFreeMemory = (funWTSFreeMemory)GetProcAddress(g_hLib, "WTSFreeMemory");
    if (g_pWTSFreeMemory == NULL)
    {
        return ;
    }    
    av_log(NULL, AV_LOG_WARNING, "gdigrab_hlib_init last \n ");     
...
static int 
gdigrab_after_lock_2sec(void )
{
    int64_t curtime;
    int64_t time_span;
    if (g_lockscreen_start_time==0)
    {
        return 0;
    }
...
static int gdigrab_read_packet(AVFormatContext *s1, AVPacket *pkt)
{
    ...
    if (!BitBlt(dest_hdc, 0, 0,
        ...
        av_log(s1, AV_LOG_INFO, "BitBlt Failed to capture image , lock screen\n");

        // 检测到可能锁屏，2秒后，暂停录屏
        g_lockscreen_start_time = av_gettime_relative();
